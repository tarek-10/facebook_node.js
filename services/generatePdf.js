...
function createInvoice(users, path) {
  let doc = new PDFDocument({ size: "A4", margin: 50 });

  generateHeader(doc);
//   generateCustomerInformation(doc, invoice);
  generateInvoiceTable(doc, users);
  generateFooter(doc);
...
function generateHeader(doc) {
  doc
    ...
    .fontSize(10)
    .text("UsersDetails.", 200, 50, { align: "right" })
...
function generateInvoiceTable(doc, users) {
  let i;
...
function generateTableRow(
  ...
  doc
    .fontSize(10)
    .text(item, 50, y)
    .text(description, 150, y,{ width: 115, align: "center" })
